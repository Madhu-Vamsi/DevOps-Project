...
def getRunningInstance():
    ...
    for eachStatus in statusOfInstances['InstanceStatuses']:
        if(eachStatus['InstanceState']['Name'] == 'running' or eachStatus['InstanceState']['Name'] == 'pending'):
            flag = True
            #print('{}\t{}'.format(eachStatus['InstanceId'], eachStatus['InstanceState']['Name']))
            runningInstance.append(eachStatus['InstanceId'])
...
file_sip = open('production-ip.txt','r')
slaveip = file_sip.readlines()[0]
...
if(True):
    ...
    instances = ec2Resource.instances.filter(Filters=[{'Name': 'instance-state-name', 'Values': ['pending']}])
    for instance in instances:
        print(instance.id, instance.instance_type)


    for allinstances in instances:
        print(allinstances)
        if(slaveip!=getIp(allinstances.id)):
            instanceip = getIp(allinstances.id)
            instanceId = allinstances.id
    ...
    file_ip = open('master_inventory','w')
